...
import Notificaton from "../Notification/notifcation";
...
const CreateDropdown = (props) => {
  ...
  const handleBucketSubmit = () => {
    if (!bucketText || options.includes(bucketText.toLowerCase())) {
      setValidationData({
        status: "invalid",
      ...
      setBucketText("");
      setValidationData({ status: "valid", message: "" });
      toast(<Notificaton title="Bucket Created" />);
    }
  };
